'''
@ 수강생 관리 시스템 데이터 저장 및 로드하기
@ 요구사항
* 메뉴 : 1. 수강생 등록 2. 수강생 목록 3. 수강생 삭제 4. 수강생 정보수정 5. 수강생 이름검색 0. 종료
* 종료 : ai_list.txt 
    -> ai_list 리스트 객체의 데이터 저장
    -> 딕셔너리 하나씩 저장, 줄바꿈
    -> ex.0번째 딕셔너리 정보 \n
    -> def save_list 작성
* 프로그램 시작
    -> read_data 호출
    -> def read_data 작성
    -> ai_list.txt readline, ai_list에 append
'''
...
def main():
    i=0
    #info_arr=[]
    info_arr = read_data("ai_list.txt")
    ...
    while(1):
        ...
        if menu == int(1):
            ...
            print("시스템 종료")
            save_list("ai_list.txt", ai_list(info_arr))
            break
        else:
